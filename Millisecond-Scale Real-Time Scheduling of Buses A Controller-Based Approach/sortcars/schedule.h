...
public:
        ...
        Constrains constrains;//初始化后，本类内不会改变。
        ...
        {               
                ...
                for (int i = 0; i < park.Cars.size(); i++)
                {
                        ...
                        //    endtime_ofday,
                        //    park.Timepoint.time, park.Timepoint.triptime,
                        //    weak_i,
                        //    sleep_type1,
                        //    sleep_type0,
                        //    constrains.H,constrains.P_used);
                ...
                for (int i = 0; i < park.Cars.size(); i++)
                {
                        ...
                        case 0://0状态代表车刚刚还在work.下面的约束条件中，可能违背多个约束，比如maxtrips和worktime 
                                ...
                                {//这些车一定trip数不满。如果trip是偶数，下班就好。如果trip是奇数，就要回溯到上个偶数trip,在那里下班。
                                        ...
                                        {
                                                if (constrains.ifbacktrack == 1)//回溯模式
                                                {
                                                        Park* backto_park = tripque[car->carx][car->tripnum - 1];
                                                        if (backto_park->tp.time >= constrains.breaktime)//此时回溯,否则回溯也解决不了问题，只能继续。
                                                        {
