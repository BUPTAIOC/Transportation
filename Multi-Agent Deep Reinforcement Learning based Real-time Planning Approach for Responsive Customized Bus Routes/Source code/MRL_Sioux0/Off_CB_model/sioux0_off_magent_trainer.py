...
import copy
from MRL_Sioux0.Off_CB_model.off_magent_model import MA_CB_RP_Sioux0, CB_Encoder
...
class StateCritic(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size):
        ...
        self.fc1 = nn.Conv1d(hidden_size * 4, 20, kernel_size=1)
    ...
    def forward(self, static, dynamic1, dynamic2, dynamic3):
        ...
        hidden = torch.cat((static_hidden, dynamic_hidden1, dynamic_hidden2, dynamic_hidden3), 1)
...
def validate(data_loader, actor, reward_fn, travel_time_G, up_station, all_station, max_loc_x, max_loc_y, render_fn=None, save_dir='.', num_plot=5):
    """Used to monitor progress on a validation set."""
    actor.eval()
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    rewards = []
    for batch_idx, batch in enumerate(data_loader):
        static, dynamic1, dynamic2, dynamic3, x0, record1, record2, record3 = batch
        ...
        if save_dir == 'test':
            start = time.time()
            with torch.no_grad():
                tour_indices, _, update_constraint, update_dynamic = actor(static, dynamic1, dynamic2, dynamic3,
    ...
    for epoch in range(20):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            reward, value_list = reward_fn(static, tour_indices, travel_time_G, all_station, update_constraint,
                                           up_station, update_dynamic, all_method_max_load, all_method_max_time, AGENT_SIZE-1)
            ...
            critic_est = critic(static, dynamic1, dynamic2, dynamic3).view(-1)
...
def train_pdptw(args, up_station, all_station, travel_time_G, Loc_x, Loc_y, up_down_index, depot, home_id, work_id, max_loc_x, max_loc_y):
    Loss_list, Reward_list = [], []
    from MRL_Sioux0.Off_CB_tasks import off_magent_pdptw
    from MRL_Sioux0.Off_CB_tasks.off_magent_pdptw import CustomizedBusDataset
    ...
    valid_data = CustomizedBusDataset(args.valid_size, args.num_nodes, max_load, MAX_DEMAND, args.seed + 1,
                                      all_station, travel_time_G, Loc_x, Loc_y, up_down_index,
                                      depot, home_id, work_id)
    actor = MA_CB_RP_Sioux0(STATIC_SIZE,
                    DYNAMIC_SIZE,
                    args.hidden_size,
                    AGENT_SIZE,
    ...
    on_madrl_out, si_out, gni_out, hr_out, rga_out, rmga_out = validate(test_loader, actor, off_magent_pdptw.reward,
