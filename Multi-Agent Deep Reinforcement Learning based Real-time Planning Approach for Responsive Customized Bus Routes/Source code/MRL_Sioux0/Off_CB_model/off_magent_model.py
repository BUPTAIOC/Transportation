...
class MA_CB_RP_Sioux0(nn.Module):
    ''' ---------------------------------------
    ...
    def __init__(self, static_size, dynamic_size, hidden_size, Agent_n,
        ...
        super(MA_CB_RP_Sioux0, self).__init__()
    ...
    def forward(self, static, dynamic1, dynamic2, dynamic3, record1, record2, record3,
        ...
        dynamic0 = [dynamic1.clone(), dynamic2.clone(), dynamic3.clone()]
        decision_list = [[] for i in range(len(static[:]))] # Agents decision list
        # decision_point = torch.ones(len(static[:]), 3, len(all_station)) * np.inf  # 决策点序列
        ...
        dynamic = [dynamic1.clone(), dynamic2.clone(), dynamic3.clone()]
        for _ in range(max_steps):
            ...
            dynamic_uod = [dynamic1.clone(), dynamic2.clone(), dynamic3.clone()]
            ...
            mask_judge_x = agent_mask1.clone() + agent_mask2.clone() + agent_mask3.clone()
        ...
        return tour_idx, tour_logp, [record1.clone(), record2.clone(), record3.clone()], [dynamic1.clone(), dynamic2.clone(), dynamic3.clone()]
