...
import copy
from MRL_Major.Off_CB_model.off_magent_model import MA_CB_RP_Major, CB_Encoder
...
class StateCritic(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size):
        ...
        self.dynamic_encoder6 = CB_Encoder(dynamic_size, hidden_size)
        self.dynamic_encoder7 = CB_Encoder(dynamic_size, hidden_size)
        self.dynamic_encoder8 = CB_Encoder(dynamic_size, hidden_size)
        self.dynamic_encoder9 = CB_Encoder(dynamic_size, hidden_size)
        self.dynamic_encoder10 = CB_Encoder(dynamic_size, hidden_size)
        self.fc1 = nn.Conv1d(hidden_size * 11, 20, kernel_size=1)
    ...
    def forward(self, static, dynamic1, dynamic2, dynamic3, dynamic4, dynamic5, dynamic6, dynamic7, dynamic8, dynamic9, dynamic10):
        ...
        dynamic_hidden6 = self.dynamic_encoder6(dynamic6)
        dynamic_hidden7 = self.dynamic_encoder7(dynamic7)
        dynamic_hidden8 = self.dynamic_encoder8(dynamic8)
        dynamic_hidden9 = self.dynamic_encoder9(dynamic9)
        dynamic_hidden10 = self.dynamic_encoder10(dynamic10)

        hidden = torch.cat((static_hidden, dynamic_hidden1, dynamic_hidden2, dynamic_hidden3, dynamic_hidden4,
                            dynamic_hidden5, dynamic_hidden6, dynamic_hidden7, dynamic_hidden8,
                            dynamic_hidden9, dynamic_hidden10), 1)  # 将静态张量和动态张量拼接在一起，得到隐藏变量
...
def validate(data_loader, actor, reward_fn, travel_time_G, up_station, all_station, max_loc_x, max_loc_y, render_fn=None, save_dir='.', num_plot=5):
    ...
    for batch_idx, batch in enumerate(data_loader):
        static, dynamic1, dynamic2, dynamic3, dynamic4, dynamic5, \
        dynamic6, dynamic7, dynamic8, dynamic9, dynamic10, x0, record1, record2, record3 = batch
        ...
        dynamic6 = dynamic6.to(device)
        dynamic7 = dynamic7.to(device)
        dynamic8 = dynamic8.to(device)
        dynamic9 = dynamic9.to(device)
        dynamic10 = dynamic10.to(device)
        ...
        if save_dir == 'test':
            start = time.time()
            with torch.no_grad():
                tour_indices, _, update_constraint, actor_dynamic = actor(static, dynamic1, dynamic2, dynamic3, dynamic4, dynamic5,
                                                                          dynamic6, dynamic7, dynamic8, dynamic9, dynamic10,
                                                                          record1, record2, record3, travel_time_G,
                                                                          up_station, all_station, x0)
            end = time.time()
            reward, value_list = reward_fn(static, tour_indices, travel_time_G, all_station, update_constraint, up_station, actor_dynamic, all_method_max_load, all_method_max_time, AGENT_SIZE-1)  # 奖励
...
def train(actor, critic, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    AGENT_SIZE = 11 # Number of agents=(AGENT_SIZE-1)
    ...
    for epoch in range(20):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            now = '%s' % datetime.datetime.now().time()
            now = now.replace(':', '_')
            tour_indices, tour_logp, update_constraint, actor_dynamic = actor(static, dynamic1, dynamic2, dynamic3,
                                                                              dynamic4, dynamic5, dynamic6, dynamic7,
                                                                              dynamic8, dynamic9, dynamic10, record1,
                                                                              record2, record3, travel_time_G,
                                                                              up_station, all_station, x0)
            ...
            reward, value_list = reward_fn(static, tour_indices, travel_time_G, all_station, update_constraint,
                                           up_station, actor_dynamic, all_method_max_load, all_method_max_time,
                                           AGENT_SIZE-1)
            ...
            critic_est = critic(static, dynamic1, dynamic2, dynamic3, dynamic4, dynamic5, dynamic6,
                                dynamic7, dynamic8, dynamic9, dynamic10).view(-1)  # 奖励估计
            advantage = (reward - critic_est)  # 差值
...
def train_pdptw(args, up_station, all_station, travel_time_G, Loc_x, Loc_y, up_down_index, depot, home_id, work_id, max_loc_x, max_loc_y):
    Loss_list, Reward_list = [], []
    from MRL_Major.Off_CB_tasks import off_magent_pdptw
    from MRL_Major.Off_CB_tasks.off_magent_pdptw import CustomizedBusDataset
    ...
    valid_data = CustomizedBusDataset(args.valid_size, args.num_nodes, max_load, MAX_DEMAND, args.seed + 1,
                                      all_station, travel_time_G, Loc_x, Loc_y, up_down_index,
                                      depot, home_id, work_id)
    actor = MA_CB_RP_Major(STATIC_SIZE,
                    DYNAMIC_SIZE,
                    args.hidden_size,
                    AGENT_SIZE,
    ...
    kwargs['reward_fn'] = off_magent_pdptw.reward
    kwargs['render_fn'] = off_magent_pdptw.render
    ...
    on_madrl_out = validate(test_loader, actor, off_magent_pdptw.reward,
                                                                        travel_time_G, up_station, all_station,
                                                                        max_loc_x, max_loc_y, off_magent_pdptw.render,
                                                                        test_dir, num_plot=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', default='off_mapdptw')
